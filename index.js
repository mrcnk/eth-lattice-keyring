...
class LatticeKeyring extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      this._unlockAndFindAccount(address)
        .then((accountIdx) => {
          if (!tx.to) {
            return reject(
              'Contract deployment is not supported by the Lattice at this time. ' +
          ...
          const forceLegacyTx =
            this.sdkSession.fwVersion[2] < 1 &&
            this.sdkSession.fwVersion[1] < 11;
          if (forceLegacyTx && txData.type === 2) {
            console.warn(
              'Lattice firmware must be >=0.11.0 to support EIP1559 transactions. Revering to legacy.'
            );
            ...
            console.warn(
              'Lattice firmware must be >=0.11.0 to support EIP2930 transactions. Reverting to legacy.'
            );
            txData.revertToLegacy = true;
            delete txData.type;
            delete txData.accessList;
        ...
        .then((signedTx) => {
          ...
          const v =
            signedTx.sig.v.length === 0 ? '0' : signedTx.sig.v.toString('hex');
          ...
          let validatingTx;
          const _chainId = `0x${this._getEthereumJsChainId(tx).toString(
            'hex'
          )}`;
          const chainId = new BigNumber(_chainId).toNumber();
          const customNetwork = Common.forCustomChain(
            'mainnet',
            {
              name: 'notMainnet',
              networkId: chainId,
    ...
    return new Promise((resolve, reject) => {
      this._unlockAndFindAccount(address)
        .then((accountIdx) => {
          ...
          if (!this._hasSession())
            return reject(
              new Error('No SDK session started. Cannot sign transaction.')
            );
          this.sdkSession.sign(req, (err, res) => {
            if (err) return reject(new Error(err));
            if (!res.sig) return reject(new Error('No signature returned'));
    ...
    return new Promise((resolve, reject) => {
      this.unlock()
        ...
        .then((addrs) => {
          ...
          if (accountIdx === null) return reject('Signer not present');
          // Make sure the account is associated with the current wallet
          if (
            this.accountOpts[accountIdx].walletUID !==
            this._getCurrentWalletUID()
          ) {
            return reject(
              new Error(
  ...
  _openConnectorTab(url) {
    return new Promise((resolve, reject) => {
      ...
      if (browserTab) {
        ...
        return reject(
          new Error('Unknown browser context. Cannot open Lattice connector.')
        );
      }
    });
  }
  ...
  __fetchAddresses(n = 1, i = 0, cb, recursedAddrs = []) {
    ...
    this.sdkSession.getAddresses(addrData, (err, addrs) => {
      ...
      if (addrs.length < 1) return cb(new Error('No addresses returned'));
      // Return the addresses we fetched *without* updating state
      if (shouldRecurse) {
        return this.__fetchAddresses(
          n - 1,
          i + 1,
          cb,
